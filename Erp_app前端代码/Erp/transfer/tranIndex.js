define(function(require){
	...
	Model.prototype.submitBtnClick = function(event){
		...
		justep.Baas.sendRequest({
			"url" : "/Erp/ErpServices",
			"action" : "getTransportTask",
			"async" : false,
			"params" : {tranId:this.comp('tranId').val()},
			"success" : function(data) {
				if(data.head.rspCode==0){
					me.comp("tranLineDialog").open({data:{tranId:data.body.TaskHead.ROW.TRANSPORT_TASK_ID,transportTaskLine:data.body.TaskBody}});
				}else{
					alert(data.msg);
				}
			}
