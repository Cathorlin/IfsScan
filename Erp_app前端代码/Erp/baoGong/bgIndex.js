define(function(require){
	...
	Model.prototype.scanBtnClick = function(event){
		...
		function onSuccess(result) {
			if(result.text!=null && result.text!=""){
				...
				me.comp('soInfo').set({value: result.text});
			}
		}
		
		function onError(error) {
			me.comp('soInfo').set({value: "扫描失败！" + error});
		}
	...
	Model.prototype.submitBtnClick = function(event){
		//var result = false;
		var soInfo = this.comp("soInfo").get("value");
		this.comp("bgLineInfoDialog").open({data:{OrderNo:soInfo.split("|")[0],ReleaseNo:soInfo.split("|")[1],SequenceNo:soInfo.split("|")[2],OperationNo:this.comp("operationNo").get("value")}});
		/*if(result){
