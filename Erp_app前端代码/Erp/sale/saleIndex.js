define(function(require){
	...
	Model.prototype.scanBtnClick = function(event){
		...
		function onSuccess(result) {
			if(result.text!=null && result.text!=""){
				...
				me.comp('shipmentId').set({value: result.text});
			}
		}
		
		function onError(error) {
			me.comp('shipmentId').set({value: "扫描失败！" + error});
		}
	...
	Model.prototype.submitBtnClick = function(event){
		...
		justep.Baas.sendRequest({
			...
			"action" : "getShipment",
			"async" : false,
			"params" : {shipmentId:this.comp('shipmentId').val()},
			"success" : function(data) {
				if(data.head.rspCode==0){
					localStorage.setItem("shipmentLine", JSON.stringify(data.body.PickList));
					me.comp("saleLineDialog").open({data:{shipmentId:data.body.PickList[0].PICK_LIST_NO,order_no:data.body.PickList[0].ORDER_NO,customerName:data.body.PickList[0].CUSTOMER_NAME}});
				}else{
